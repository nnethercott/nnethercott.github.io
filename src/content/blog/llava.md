...
To show the effect of each different technique I prompted the quantized model to answer the same question we were considering before and benchmarked decoding speed and memory footprint.  You can see the tradeoff between model size and accuracy/coherence when looking at the generations for the tiny quants vs the larger ones. 

My personal favourite is the quant generated by AWQ due to its fast decoding speed and humble size of 4.5GB.  That being said I think its incredibly cool that the `q2_k` version is still coherent even despite how small we compressed it, even if it did hallucinate a bit.
